...
def penalidade_notas(all_salas, all_alunos_data):
    medias = []

    for sala_ids in all_salas.values():
        media = aval_notas(sala_ids, all_alunos_data)
        medias.append(media)
...
if arquivo is not None:
    try:
        arquivo.seek(0)
        df_verificacao = pd.read_csv(arquivo, sep=None, engine='python')
        ...
        coluna_id_encontrada = (
            'ID' in df_verificacao.columns or
            'id' in df_verificacao.columns
        )

        faltando = [
            coluna
            for coluna in colunas_obrigatorias
            if coluna not in df_verificacao.columns
        ]

        if not coluna_id_encontrada:
            faltando.append('ID (ou id)')

        if faltando:
            st.error(
                "⚠️ CSV inválido! Colunas faltando: "
                + ", ".join(faltando)
            )
            st.stop()
        ...
        st.error(f"⚠️ Erro ao verificar o arquivo: {e}")
        st.stop()
